...
// Register a user (similar to createUser but with automatic 'member' idType)
void UserController::registerUser() {
    ...
    // Generate user ID and ID type (auto-generated user ID and 'member' type)
    std::string userID = generateUserID();
    std::string idType = "member"; // Always set to 'member' for registration
...
std::string UserController::generateUserID() {
    std::vector<User> users = userDAO.getAllUsers();
    int newUserId = users.size() + 1; // Assume user IDs are sequential and start from 1
    std::stringstream ss;
